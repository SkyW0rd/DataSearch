...
> [!IMPORTANT]
> Собрано и реально протестировано пока только на **macOS**. Реализации для Windows и Linux написаны по документации соответствующих API, но ещё не собирались и не запускались вживую — см. [статус по платформам](#-статус-по-платформам).
...
| **Windows** | ⚠️ Реализовано (`ReadDirectoryChangesW`, `ShellExecuteW`, `SHOpenFolderAndSelectItems`), но не собиралось и не запускалось — нет Windows-машины в среде разработки на момент написания. Требует сборки и проверки на реальной Windows-машине перед использованием. |
